...
DEFAULT_OLLAMA_HOST = "http://llm.ijs.si:11435"
...
PLM_PROBABILITIES_KEY = "global-context-modelling/simplified-dart-xlmr/probabilities" # New key for softmax
...
def load_data(language: str, split_type: str, split_index: Optional[int] = None) -> List[Dict[str, Any]]:
    base_data_root = "../data/final/balanced-predicted-softmax/" # Updated data root
...
def run_single_item_direct_style(
    ...
    # For direct methods, raw_responses_agg is used.
    ...
    for _ in range(num_queries):
        raw_response = ollama_request_sync(prompt, model_name, ollama_url, temperature, top_k, top_p, max_retries, retry_delay, timeout, 20) # 20 tokens for direct response
...
The reasoning should explain how the PLM suggestion was considered."""
    ...
    # --- Optional Signatures with PLM Reliability Information AND Softmax ---
    class PLMAugmentedWithReliabilityAndSoftmaxSignature(dspy.Signature):
        """Given an article, aspect, a PLM sentiment suggestion, PLM reliability info,
    and PLM softmax probabilities, classify the final sentiment ('negative', 'neutral', 'positive').
    Critically evaluate the PLM suggestion using all provided context."""
        article: str = dspy.InputField(desc="The full text of the article.")
        aspect: str = dspy.InputField(desc="The specific aspect.")
        plm_suggestion: TypingLiteral["negative", "neutral", "positive"] = dspy.InputField(desc="Sentiment suggestion from a prior model.")
        plm_reliability_info: str = dspy.InputField(desc="Information about the PLM's typical F1 scores for the suggested class and language.")
        ...
        sentiment: TypingLiteral["negative", "neutral", "positive"] = dspy.OutputField(desc="Final sentiment after considering all PLM information.")
...
if DSPY_AVAILABLE:
    def configure_dspy_lm(model_name_str: str, host_url: str, temperature: float, top_k: int, top_p: float, dspy_max_tokens: int, role: str = "student") -> Optional[dspy.LM]:
        try:
            # Reverted to the dspy.LM syntax that was working in the original script
            lm = dspy.LM(
                ...
                top_p=top_p, 
                top_k=top_k, 
                max_tokens=dspy_max_tokens # For dspy.LM, max_tokens controls num_predict
            )
    ...
    def prepare_dspy_dataset(data: List[Dict[str, Any]], method_name: str, language: str, use_plm_reliability_sig: bool, use_plm_softmax_sig: bool, purpose: str = "training") -> List[dspy.Example]:
        dspy_dataset = []
            ...
            if article and aspect and sentiment_int is not None:
                try:
                    ...
                    if needs_plm_input_field:
                        ...
                        if use_plm_reliability_sig: # This will be true if use_plm_softmax_sig is true
                            f1_scores_lang = PLM_F1_SCORES.get(language, {})
                            f1_neg = f1_scores_lang.get("negative", "N/A")
                            f1_neu = f1_scores_lang.get("neutral", "N/A")
                            f1_pos = f1_scores_lang.get("positive", "N/A")
                            ...
                            if use_plm_softmax_sig:
                                ...
                                if plm_probs_dict and isinstance(plm_probs_dict, dict):
                                    # Ensure these keys ("Negative", "Neutral", "Positive") exactly match your JSON structure
                                    ...
                                    logging.debug(f"Item {item.get('uuid')} missing or malformed PLM probabilities ({PLM_PROBABILITIES_KEY}) for softmax signature during dataset prep. Using N/A.")
        ...
        logging.info(f"Prepared {len(dspy_dataset)} DSPy {purpose} examples for method '{method_name}' (PLM reliability sig: {use_plm_reliability_sig}, PLM softmax sig: {use_plm_softmax_sig}).")
        return dspy_dataset
    ...
    def optimize_dspy_program_mipro(
        ...
        autorun_setting: str, miprov2_init_temp: float # autorun_setting is kept for consistency but not directly used by MIPROv2 compile
        ...
        # MIPROv2 parameters based on typical usage.
        # The 'auto' parameter from older optimizers is not directly part of MIPROv2.
        # 'max_errors' is also not a direct parameter for MIPROv2's compile method.
        optimizer = MIPROv2(metric=validate_sentiment, 
                            prompt_model=prompt_teacher_lm, 
                            task_model=student_lm, 
                            max_bootstrapped_demos=0, # As per original script's intent
                            max_labeled_demos=0,    # As per original script's intent
                            # num_candidates=10, # Default is 10, can be exposed as arg
                            # max_threads=8,     # Default is 10, can be exposed as arg
        ...
        try:
            # The 'requires_permission_to_run' is not a parameter for MIPROv2.compile
    ...
    def run_dspy_prediction(items_to_process: List[Dict[str, Any]], optimized_program: dspy.Module, num_queries: int, logs_dir: str, method_name: str, language: str, use_plm_reliability_sig: bool, use_plm_softmax_sig: bool) -> List[Dict[str, Any]]:
        newly_processed_item_logs = []
        for item in tqdm(items_to_process, desc=f"DSPy Prediction ({method_name})"):
            item_uuid = item['uuid']
            article, aspect = item.get('article', ''), item.get('aspect', '')
            # For DSPy methods, dspy_query_details will store detailed query outputs.
            ...
            if "plm-augmented" in method_name:
                ...
                if use_plm_reliability_sig: # True if use_plm_softmax_sig is true
                    f1_scores_lang = PLM_F1_SCORES.get(language, {})
                    f1_neg = f1_scores_lang.get("negative", "N/A")
                    f1_neu = f1_scores_lang.get("neutral", "N/A")
                    f1_pos = f1_scores_lang.get("positive", "N/A")
                    ...
                    if use_plm_softmax_sig:
                        ...
                        if plm_probs_dict and isinstance(plm_probs_dict, dict):
                            ...
                            softmax_str_for_call = (
                             ...
                             logging.debug(f"Item {item_uuid} missing or malformed PLM probabilities ({PLM_PROBABILITIES_KEY}) for softmax signature during prediction. Using N/A.")
            ...
            for query_idx in range(num_queries):
                ...
                try:
                    ...
                    query_detail["raw_prediction_object_str"] = str(prediction_obj) # Store full object string
                    ...
                    if pred_str and pred_str.lower() in LABELS_STR:
                        ...
                        query_success_flag = True # At least one query was successful in parsing
                        query_detail["status"] = "success"
                        query_detail["predicted_sentiment"] = pred_str.lower()
                    else:
                        query_detail["status"] = "invalid_label"
                        query_detail["predicted_sentiment"] = pred_str # Store whatever was returned
            ...
            current_item_log = {
                ...
                "dspy_query_details": item_query_details_list, # Store detailed list
...
def save_final_results(target_dataset_subset: List[Dict[str, Any]], all_item_log_results_for_subset: List[Dict[str, Any]], metrics_summary: Dict[str, Any], output_dir: str, language_prefix: str, debug_run: bool, custom_run_name: Optional[str]):
    ...
    for original_item in target_dataset_subset:
        ...
        if item_uuid in results_map:
            ...
            elif "raw_responses_agg" in logged_result: 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dspy-autorun", type=str, default=DEFAULT_DSPY_AUTORUN, choices=["light", "medium", "heavy"], help="DSPy MIPROv2 auto mode setting (Note: MIPROv2 doesn't directly use this; kept for potential future use/config).")
    ...
    parser.add_argument("--use-plm-softmax-signature", action="store_true", help="Use DSPy Signatures for PLM-augmented methods that include PLM softmax probabilities. Implies --use-plm-reliability-signature.")
    ...
    if args.use_plm_softmax_signature:
        if not ("plm-augmented" in args.method):
            logging.error("--use-plm-softmax-signature is only compatible with 'dspy-plm-augmented' or 'dspy-plm-augmented-cot' methods.")
            exit(1)
        if not args.use_plm_reliability_signature:
            logging.info("--use-plm-softmax-signature is active; automatically enabling --use-plm-reliability-signature as softmax is an extension of reliability info.")
            args.use_plm_reliability_signature = True 
    
    if args.use_plm_reliability_signature and not ("plm-augmented" in args.method):
        logging.error("--use-plm-reliability-signature is only compatible with 'dspy-plm-augmented' or 'dspy-plm-augmented-cot' methods unless --use-plm-softmax-signature is also set (which implies PLM method).")
        exit(1)
        
    if args.use_aspect_marker_signature and ("plm-augmented" in args.method):
        logging.warning("--use-aspect-marker-signature is typically for non-PLM-augmented methods. It will be ignored if method is PLM-augmented and --use-plm-reliability-signature (or softmax) is also set, as those take precedence for PLM methods.")
    ...
    if args.use_aspect_marker_signature: logging.info("Using Aspect Marker Signature variant for DSPy (if applicable).")
    if args.use_plm_reliability_signature: logging.info("Using PLM Reliability Signature variant for DSPy (if applicable).")
    if args.use_plm_softmax_signature: logging.info("Using PLM Softmax Signature variant for DSPy (if applicable, implies reliability).")
    ...
    if args.method == 'direct':
        ...
        if args.method == "dspy-predict":
            ...
            if args.use_plm_softmax_signature: 
                signature_class_to_use = PLMAugmentedWithReliabilityAndSoftmaxSignature
            elif args.use_plm_reliability_signature:
                signature_class_to_use = PLMAugmentedWithReliabilitySignature
            else:
                signature_class_to_use = PLMAugmentedSentimentSignature
        elif args.method == "dspy-plm-augmented-cot":
            if args.use_plm_softmax_signature: 
                signature_class_to_use = ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature
            elif args.use_plm_reliability_signature:
                signature_class_to_use = ReasoningPLMAugmentedWithReliabilitySignature
            else:
                signature_class_to_use = ReasoningPLMAugmentedSentimentSignature
        ...
        is_reliability_sig_active_for_filename = args.use_plm_reliability_signature and signature_class_to_use in [
            PLMAugmentedWithReliabilitySignature, ReasoningPLMAugmentedWithReliabilitySignature,
            PLMAugmentedWithReliabilityAndSoftmaxSignature, ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature
        ]
        if is_reliability_sig_active_for_filename:
             optimized_program_filename += "_plmreliability"
        
        if args.use_plm_softmax_signature and signature_class_to_use in [PLMAugmentedWithReliabilityAndSoftmaxSignature, ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature]:
            optimized_program_filename += "_plmsoftmax"

        if args.teacher_model_short_name: optimized_program_filename += f"_teacher_{args.teacher_model_short_name}"
        optimized_program_filename += f"_autorun_{args.dspy_autorun}_temp_{args.miprov2_temp}.json" # autorun is just for filename convention here
        ...
        if not program_to_run:
            ...
            dspy_trainset = prepare_dspy_dataset(train_data, args.method, args.split, args.use_plm_reliability_signature, args.use_plm_softmax_signature, "training")
            dspy_valset = prepare_dspy_dataset(val_data, args.method, args.split, args.use_plm_reliability_signature, args.use_plm_softmax_signature, "validation")
        ...
        if program_to_run and items_to_process_this_run:
            newly_processed_item_logs = run_dspy_prediction(items_to_process_this_run, program_to_run, args.num_queries, logs_dir, args.method, args.split, args.use_plm_reliability_signature, args.use_plm_softmax_signature)
    ...
    if is_dspy_method and signature_class_to_use: 
        is_aspect_marker_sig_active = args.use_aspect_marker_signature and signature_class_to_use in [
            SentimentClassificationWithAspectMarkerSignature, ReasoningSentimentWithAspectMarkerSignature
        ]
        is_reliability_sig_active = args.use_plm_reliability_signature and signature_class_to_use in [
            PLMAugmentedWithReliabilitySignature, ReasoningPLMAugmentedWithReliabilitySignature,
            PLMAugmentedWithReliabilityAndSoftmaxSignature, ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature
        ]
        is_softmax_sig_active = args.use_plm_softmax_signature and signature_class_to_use in [
            PLMAugmentedWithReliabilityAndSoftmaxSignature, ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature
        ]
        metrics_summary.update({
            ...
            "dspy_used_aspect_marker_signature": is_aspect_marker_sig_active,
            "dspy_used_plm_reliability_signature": is_reliability_sig_active,
            "dspy_used_plm_softmax_signature": is_softmax_sig_active,
