...
parser.add_argument("--max_seq_length", type=int, default=96, help="Max token sequence length for EACH sentence.") # Reduced
parser.add_argument("--max_sentences", type=int, default=32, help="Max number of sentences per document.") # Reduced
...
parser.add_argument("--aggregation_heads", type=int, default=4, help="Number of attention heads for global aggregation.") # Renamed from DARTStyleModel
...
parser.add_argument("--batch_size", type=int, default=2, help="Micro-batch size (adjust based on GPU memory).") # Reduced
parser.add_argument("--eff_batch_size_target", type=int, default=32, help="Target effective batch size via gradient accumulation.")
parser.add_argument("--lr", type=float, default=1e-5, help="Learning rate.") # Potentially reduced
...
AGGREGATION_HEADS = args.aggregation_heads # Used for the single global aggregation
FINAL_MLP_HIDDEN_DIM = args.final_mlp_hidden_dim
DROPOUT_RATE = args.dropout_rate
USE_ASPECT_MARKER = args.use_aspect_marker
FREEZE_BASE_MODEL = args.freeze_base_model
EPOCHS = args.epochs
BATCH_SIZE = args.batch_size # This is micro-batch size
...
class SentimentSentenceDataset(Dataset):
    ...
    def _replace_aspect_with_placeholder(self, text):
        start_idx = text.find(self.aspect_tag_start)
        end_idx = text.find(self.aspect_tag_end)
        # Iteratively replace to handle multiple aspects if they were to occur (though problem implies one)
        while start_idx != -1 and end_idx != -1 and start_idx < end_idx:
            ...
            start_idx = text.find(self.aspect_tag_start) # Find next occurrence
            end_idx = text.find(self.aspect_tag_end)
        # Clean any remaining unmatched tags (less likely if data is clean)
    ...
    def __getitem__(self, idx):
        ...
        article_with_placeholder = self._replace_aspect_with_placeholder(raw_article)

        try:
            sentences = split_document(article_with_placeholder, worker_spacy_nlp)
        except Exception as e:
...
def main():
    ...
    for i in range(3):
        ...
        try:
            ...
            if isinstance(loaded_args_dict, dict):
                loaded_args_ns = argparse.Namespace(**loaded_args_dict)
            else: 
                loaded_args_ns = loaded_args_dict
        except Exception as e:
        ...
        try:
            model_test = SimplifiedDARTModel(
                ...
                aggregation_heads=getattr(loaded_args_ns, 'aggregation_heads', AGGREGATION_HEADS), # For older checkpoints
    ...
    if test_results_summary:
        # Filter out None values before calculating mean, in case a run failed to produce metrics
        ...
        print(f"Avg Macro F1: {avg_test_f1_macro:.4f} (from {len(f1_scores)} models)")
        print(f"Avg Accuracy: {avg_test_accuracy:.4f} (from {len(acc_scores)} models)")
        print(f"Avg QWK: {avg_test_qwk:.4f} (from {len(qwk_scores)} models)")
        all_test_metrics["average_performance"] = {
            "f1_macro": avg_test_f1_macro, "accuracy": avg_test_accuracy, "qwk": avg_test_qwk,
            "num_models_averaged_f1": len(f1_scores) # Example of more detailed reporting
        }
    else:
