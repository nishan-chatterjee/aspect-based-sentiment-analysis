...
BASE_OUTPUT_DATA_DIR_TEMPLATE = "../data/final/balanced-predicted-softmax/{method_slug}"
    ...
    for file_config in input_files_config:
        ...
        if language not in loaded_models_cache:
            ...
            model = SimplifiedDARTModel(
                ...
                num_classes=3, freeze_base=False # num_classes=3 for Neg/Neu/Pos
            ).to(device)
            ...
            loaded_models_cache[language] = (model, tokenizer, torch.tensor(aspect_target_token_id, device=device), args_ns, spacy_nlp_for_lang) # Pass scalar tensor
        ...
        model, tokenizer, aspect_id_scalar_tensor, args_ns, spacy_nlp = cached_data
            ...
            for item_ref in tqdm(all_items_to_process, desc=f"Predicting for {input_filename}"):
                try:
                    ...
                    with torch.no_grad():
                        ...
                        probabilities_tensor = F.softmax(logits, dim=1).cpu().squeeze() # Shape (num_classes,) e.g., (3,)
                        
                        prediction_idx = torch.argmax(probabilities_tensor).item() # Index 0, 1, or 2
                        sentiment_prediction = prediction_idx - 1 # Maps to -1, 0, 1
                        
                        # Create probabilities dictionary
                        # Assumes class 0: Negative, class 1: Neutral, class 2: Positive
                        probabilities_list = probabilities_tensor.tolist()
                        prob_dict = {
                            "Negative": probabilities_list[0], 
                            "Neutral": probabilities_list[1],
                            "Positive": probabilities_list[2]
                        }
                    
                    item_ref[METHOD_KEY_FOR_JSON] = sentiment_prediction 
                    item_ref[METHOD_PROB_KEY_FOR_JSON] = prob_dict
                    ...
                    item_ref[METHOD_PROB_KEY_FOR_JSON] = "ERROR_DURING_PROCESSING" # Also indicate error for probs
