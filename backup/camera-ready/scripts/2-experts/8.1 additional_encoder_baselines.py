#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""additional-comparison encoder baselines for document-level ABSA.

Runs one model family for one language and one aspect setting. For each run it
trains three split-specific checkpoints, selects by validation macro F1, and
evaluates each checkpoint on the fixed test split. The output JSON layout is
kept compatible with notebooks/7.0 result-analysis.ipynb.
"""
...
DEFAULT_MODEL_DIRS = {
    "longformer": "markussagen_xlm-roberta-longformer-base-4096",
    "mdeberta": "microsoft_mdeberta-v3-base",
    "slavic_specific:slovenian": "EMBEDDIA_sloberta",
    "slavic_specific:serbian": "classla_bcms-bertic",
}


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train additional-comparison encoder baselines on ABSA splits."
    )
    parser.add_argument(
        "--approach",
        required=True,
...
def default_model_path(args):
    ...
    if args.approach == "slavic_specific":
        key = "slavic_specific:%s" % args.split
    else:
        key = args.approach
    return Path(args.model_root) / DEFAULT_MODEL_DIRS[key]


def default_max_len(approach):
    return 4096 if approach == "longformer" else 512
...
class EncoderCollator:
    def __init__(self, tokenizer, max_len, use_global_attention):
        self.tokenizer = tokenizer
        self.max_len = max_len
        self.use_global_attention = use_global_attention

    def __call__(self, features):
        articles = [f["article"] for f in features]
        aspects = [f["aspect"] for f in features]
...
def build_longformer_classifier(model_path):
    ...
    if not weights_path.exists():
        print(
            "Warning: %s not found. Initializing Longformer from config only."
            % weights_path
        )
        return model

    state = torch_load(weights_path, map_location="cpu")
    ...
    for key, value in state.items():
        if key.startswith("roberta."):
            renamed_state["longformer." + key[len("roberta.") :]] = value
        elif key.startswith("longformer."):
            renamed_state[key] = value

    missing, unexpected = model.load_state_dict(renamed_state, strict=False)
    missing_classifier = [m for m in missing if m.startswith("classifier.")]
    missing_other = [m for m in missing if not m.startswith("classifier.")]
    print(
        "Loaded Longformer base weights. Missing classifier params: %s; other missing: %s; unexpected: %s"
        % (len(missing_classifier), len(missing_other), len(unexpected))
    )
    if missing_other:
        print("First non-classifier missing keys: %s" % missing_other[:8])
    if unexpected:
        print("First unexpected keys: %s" % unexpected[:8])
    return model
...
def load_tokenizer_and_model(args, model_path):
    if args.approach == "slavic_specific" and args.split == "slovenian":
        ...
        print("Loaded Slovenian SloBERTa tokenizer from tokenizer.json.")
    else:
        tokenizer = AutoTokenizer.from_pretrained(
            model_path, local_files_only=args.local_files_only, use_fast=True
        )

    if args.approach == "longformer":
        model = build_longformer_classifier(model_path)
    else:
        config = AutoConfig.from_pretrained(
            model_path, local_files_only=args.local_files_only, num_labels=3
        )
        ...
        if args.dropout_rate is not None:
            for attr in (
                ...
                if hasattr(config, attr):
                    setattr(config, attr, args.dropout_rate)
    ...
    model.float()
    assert_finite_model_parameters(model, "Loaded %s model" % args.approach)
...
def make_loader(dataset, tokenizer, args, shuffle, batch_size=None):
    collator = EncoderCollator(
        tokenizer=tokenizer,
        max_len=args.max_len,
        use_global_attention=args.approach == "longformer",
    )
    return DataLoader(
        dataset,
...
def evaluate(model, dataloader, device, use_amp, desc="Evaluating"):
    model.eval()
    total_loss = 0.0
    total_items = 0
    all_preds = []
...
def train_one_run(
    ...
    print("\n--- Starting training for run %s ---" % run_index)
...
def main():
    ...
    args = parse_args()
    args.max_len = args.max_len or default_max_len(args.approach)
    set_seed(args.seed)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_path = default_model_path(args)
    variant_name = "masked" if args.mask_aspect else "unmasked"
    output_dir = Path(args.output_root) / args.approach / variant_name / args.split
    ...
    print("--- Comparison Encoder Baseline ---")
    print("Approach: %s" % args.approach)
    ...
    print("Max length: %s" % args.max_len)
    ...
    if not args.test_only:
        for run_index in args.run_indices:
            ...
            train_dataset = ABSADataset(train_raw, args.mask_aspect)
            val_dataset = ABSADataset(val_raw, args.mask_aspect)
    ...
    for run_index in args.run_indices:
        ...
        test_dataset = ABSADataset(test_raw, args.mask_aspect)
