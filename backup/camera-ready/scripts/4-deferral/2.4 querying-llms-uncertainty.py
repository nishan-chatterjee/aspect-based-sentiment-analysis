# -*- coding: utf-8 -*-
# Filename: 2.4_querying-llms-uncertainty.py
import json
import argparse
import os
import time
...
DEFAULT_DSPY_MAX_TOKENS = 1024
DEFAULT_MIPROV2_TEMP = 1.0
DEFAULT_DSPY_AUTORUN = "heavy"

# --- Keys for accessing PLM data from JSON ---
...
PLM_UNCERTAINTY_KEY = "global-context-modelling/simplified-dart-xlmr/uncertainty"

# --- Label and Token Constants ---
...
# --- Prompting Strategy Information ---
# PLM F1 scores for 'reliability' strategy
PLM_F1_SCORES = {
    "slovenian": {"negative": 46.05, "neutral": 94.14, "positive": 74.51},
    "serbian": {"negative": 69.84, "neutral": 84.36, "positive": 87.39}
}

# High-level facts about PLM calibration for 'uncertainty' strategy
...
def load_data(language: str, split_type: str, split_index: Optional[int] = None) -> List[Dict[str, Any]]:
    # NOTE: The path points to the new data source with uncertainty metrics
    base_data_root = "../data/final/balanced-predicted-softmax-uncertainty/"
    ...
    try:
        ...
        # Helper to recursively find items with predictions
        def find_items(node, items_list):
            if isinstance(node, dict):
                if 'uuid' in node and PLM_PREDICTION_KEY in node:
                    items_list.append(node)
                else:
                    for k in node:
                        find_items(node[k], items_list)
            elif isinstance(node, list):
                for item in node:
                    find_items(item, items_list)
        
        data = []
        find_items(content, data)

        if not data:
            logging.warning(f"Loaded data is empty from {file_path}.")
            return []
        else: logging.info(f"Loaded {len(data)} items for {language} (from {filename}).")
...
# 'reliability' strategy signature
...
# 'uncertainty' strategy signature
class ReasoningPLMAugmentedWithUncertaintySignature(dspy.Signature):
...
2. Conclude with the final sentiment ('negative', 'neutral', 'positive')."""
    ...
    aspect: str = dspy.InputField(desc="The specific aspect. This will either be the specific aspect phrase or the generic placeholder '[MASK]' if the aspect's name has been hidden in the article.")
    plm_suggestion: TypingLiteral["negative", "neutral", "positive"] = dspy.InputField(desc="Sentiment suggestion from a prior model.")
    plm_confidence_score: str = dspy.InputField(desc="The PLM's confidence in its suggestion (e.g., 'Confidence: 60.0%'). This reflects the proportion of internal model votes for the suggestion.")
    plm_predictive_entropy: str = dspy.InputField(desc="A measure of the PLM's internal disagreement. Higher values (e.g., >1.0) mean more uncertainty. (e.g., 'Predictive Entropy: 0.989').")
    plm_calibration_fact: str = dspy.InputField(desc="A general fact about the PLM's typical calibration behavior for this language (e.g., whether it tends to be overconfident).")
    reasoning: str = dspy.OutputField(desc="Step-by-step reasoning, incorporating the PLM suggestion and all provided uncertainty metrics.")
    sentiment: TypingLiteral["negative", "neutral", "positive"] = dspy.OutputField(desc="Final sentiment.")
...
def prepare_dspy_dataset(data: List[Dict[str, Any]], language: str, mask_aspect: bool, prompt_strategy: str, purpose: str = "training") -> List[dspy.Example]:
    dspy_dataset = []
    for item in data:
        ...
        if article and aspect and sentiment_int is not None:
            try:
                ...
                # --- STRATEGY-BASED PROMPT FIELD GENERATION ---
                if prompt_strategy == 'uncertainty':
                    uncertainty_dict = item.get(PLM_UNCERTAINTY_KEY, {})
                    confidence = uncertainty_dict.get('confidence_score', 0.0)
                    entropy = uncertainty_dict.get('predictive_entropy', 0.0)
                    
                    example_args["plm_confidence_score"] = f"Confidence in this suggestion is {confidence:.1%}."
                    example_args["plm_predictive_entropy"] = f"The predictive entropy is {entropy:.3f}. A higher value (e.g., > 1.0) indicates more internal disagreement/uncertainty in the PLM."
                    example_args["plm_calibration_fact"] = PLM_CALIBRATION_INFO.get(language, "General Note: The PLM's calibration behavior for this language is unknown.")
                    input_keys.extend(["plm_confidence_score", "plm_predictive_entropy", "plm_calibration_fact"])

                else: # Default to 'reliability' strategy
                    f1_scores_lang = PLM_F1_SCORES.get(language, {})
                    f1_neg, f1_neu, f1_pos = f1_scores_lang.get("negative", "N/A"), f1_scores_lang.get("neutral", "N/A"), f1_scores_lang.get("positive", "N/A")
                    ...
                    example_args["plm_reliability_info"] = (f"PLM reliability for {language} (F1 scores): Negative ~{f1_neg}%, Neutral ~{f1_neu}%, Positive ~{f1_pos}%. Current PLM suggestion '{plm_pred_str}' has reliability ~{reliability_detail}.")
                    
                    plm_probs_dict = item.get(PLM_PROBABILITIES_KEY, {})
                    prob_neg, prob_neu, prob_pos = plm_probs_dict.get("Negative", 0.0), plm_probs_dict.get("Neutral", 0.0), plm_probs_dict.get("Positive", 0.0)
                    example_args["plm_softmax_probabilities"] = (f"PLM Prediction Softmax Probabilities: Negative={prob_neg:.4f}, Neutral={prob_neu:.4f}, Positive={prob_pos:.4f}.")
                    input_keys.extend(["plm_reliability_info", "plm_softmax_probabilities"])
    ...
    logging.info(f"Prepared {len(dspy_dataset)} DSPy {purpose} examples (Strategy: {prompt_strategy}, Masking: {mask_aspect}).")
    return dspy_dataset
...
def run_dspy_prediction(items_to_process: List[Dict[str, Any]], optimized_program: dspy.Module, num_queries: int, logs_dir: str, language: str, mask_aspect: bool, prompt_strategy: str) -> List[Dict[str, Any]]:
    newly_processed_item_logs = []
    for item in tqdm(items_to_process, desc=f"DSPy Prediction (Strategy: {prompt_strategy}, Masking: {mask_aspect})"):
        item_uuid = item['uuid']
        ...
        # --- STRATEGY-BASED PROMPT FIELD GENERATION FOR PREDICTION ---
        if prompt_strategy == 'uncertainty':
            uncertainty_dict = item.get(PLM_UNCERTAINTY_KEY, {})
            confidence = uncertainty_dict.get('confidence_score', 0.0)
            entropy = uncertainty_dict.get('predictive_entropy', 0.0)
            
            call_args["plm_confidence_score"] = f"Confidence in this suggestion is {confidence:.1%}."
            call_args["plm_predictive_entropy"] = f"The predictive entropy is {entropy:.3f}. A higher value (e.g., > 1.0) indicates more internal disagreement/uncertainty in the PLM."
            call_args["plm_calibration_fact"] = PLM_CALIBRATION_INFO.get(language, "General Note: The PLM's calibration behavior for this language is unknown.")
            ...
            if plm_pred_str in f1_scores_lang and f1_scores_lang[plm_pred_str] != "N/A":
                reliability_detail = f"{f1_scores_lang[plm_pred_str]}%"
            call_args["plm_reliability_info"] = (f"PLM reliability for {language} (F1 scores): Negative ~{f1_neg}%, Neutral ~{f1_neu}%, Positive ~{f1_pos}%. Current PLM suggestion '{plm_pred_str}' has reliability ~{reliability_detail}.")

            plm_probs_dict = item.get(PLM_PROBABILITIES_KEY, {})
            prob_neg, prob_neu, prob_pos = plm_probs_dict.get("Negative", 0.0), plm_probs_dict.get("Neutral", 0.0), plm_probs_dict.get("Positive", 0.0)
            call_args["plm_softmax_probabilities"] = (f"PLM Prediction Softmax Probabilities: Negative={prob_neg:.4f}, Neutral={prob_neu:.4f}, Positive={prob_pos:.4f}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt-strategy", type=str, default="reliability", choices=["reliability", "uncertainty"], help="The type of information to include in the LLM prompt: 'reliability' (F1/softmax) or 'uncertainty' (confidence/entropy/calibration).")
    ...
    parser.add_argument("--use-plm-reliability-signature", action="store_true", help="[DEPRECATED, use --prompt-strategy reliability] Use DSPy Signatures that include PLM reliability info.")
    parser.add_argument("--use-plm-softmax-signature", action="store_true", help="[DEPRECATED, use --prompt-strategy reliability] Use DSPy Signatures that include PLM softmax probabilities.")
    ...
    if args.prompt_strategy == 'uncertainty':
        signature_class_to_use = ReasoningPLMAugmentedWithUncertaintySignature
        METHOD_NAME = "dspy-plm-augmented-cot-with-uncertainty"
    else: # Default to reliability
        signature_class_to_use = ReasoningPLMAugmentedWithReliabilityAndSoftmaxSignature
        METHOD_NAME = "dspy-plm-augmented-cot-with-reliability"
    ...
    logging.info(f"Run Config: Model='{actual_model_string}' (Short: '{args.model}'), Method='{METHOD_NAME}', Split='{args.split}', Masking='{args.mask}', Strategy='{args.prompt_strategy}', Name='{args.name if args.name else 'Default'}'")
    ...
    if not program_to_run:
        ...
        dspy_trainset = prepare_dspy_dataset(train_data, args.split, args.mask, args.prompt_strategy, "training")
        dspy_valset = prepare_dspy_dataset(val_data, args.split, args.mask, args.prompt_strategy, "validation")
    ...
    if program_to_run and items_to_process_this_run:
        newly_processed_item_logs = run_dspy_prediction(items_to_process_this_run, program_to_run, args.num_queries, logs_dir, args.split, args.mask, args.prompt_strategy)
    ...
    metrics_summary = {
        ...
        "prompting_strategy_used": args.prompt_strategy,
