...
run_primary() {
  ...
  if ! primary_enabled "$language" "$primary"; then
    echo "=== Skipping selective deferral optimize for primary=${primary} language=${language} due to PRIMARY_FILTER=${PRIMARY_FILTER} ==="
    return 0
  fi
  echo "=== Optimizing selective deferral for primary=${primary} language=${language} ==="
