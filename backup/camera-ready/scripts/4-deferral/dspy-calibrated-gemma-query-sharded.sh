#!/bin/bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="${ABSA_RELEASE_ROOT:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
cd "$ROOT_DIR"

PYTHON_BIN="${PYTHON_BIN:-$(command -v python)}"
if [[ ! -x "$PYTHON_BIN" ]]; then
  echo "Python not found or not executable: $PYTHON_BIN" >&2
  exit 2
fi

EXPERT="${EXPERT:?Set EXPERT, e.g. longformer_masked}"
LANGUAGE="${LANGUAGE:?Set LANGUAGE, e.g. serbian}"
PROMPT_VARIANT="${PROMPT_VARIANT:-masked}"
AUTORUN="${AUTORUN:-medium}"
PROMPT_STYLE="${PROMPT_STYLE:-rich}"

OUTPUT_ROOT="${OUTPUT_ROOT:-reviews/uncertainty/rich-llm-dspy-calibration-cot}"
UNCERTAINTY_ROOT="${UNCERTAINTY_ROOT:-reviews/uncertainty}"
STUDENT_MODEL="${STUDENT_MODEL:-gemma27b}"
STUDENT_API_BASES="${STUDENT_API_BASES:?Set STUDENT_API_BASES as comma-separated endpoints}"
OPENAI_API_KEY="${OPENAI_API_KEY:-EMPTY}"
TEACHER_LABEL="${TEACHER_LABEL:-qwen-2.5-72b}"

MAX_TOKENS="${MAX_TOKENS:-1024}"
TEMPERATURE="${TEMPERATURE:-1.0}"
TOP_P="${TOP_P:-0.95}"
MIPROV2_TEMP="${MIPROV2_TEMP:-1.0}"
MAX_ARTICLE_CHARS="${MAX_ARTICLE_CHARS:-10000}"
NUM_QUERIES="${NUM_QUERIES:-1}"
NUM_WORKERS_PER_ENDPOINT="${NUM_WORKERS_PER_ENDPOINT:-6}"
LIMIT_ITEMS="${LIMIT_ITEMS:-}"
SAMPLE_ITEMS="${SAMPLE_ITEMS:-}"
SEED="${SEED:-42}"
FORCE="${FORCE:-0}"
SKIP_ENDPOINT_CHECK="${SKIP_ENDPOINT_CHECK:-0}"

export OPENAI_API_KEY
export TOKENIZERS_PARALLELISM="${TOKENIZERS_PARALLELISM:-false}"
export LITELLM_LOCAL_MODEL_COST_MAP="${LITELLM_LOCAL_MODEL_COST_MAP:-true}"
export LITELLM_LOG="${LITELLM_LOG:-ERROR}"
export DSPY_CACHEDIR="${DSPY_CACHEDIR:-${OUTPUT_ROOT}/.dspy_cache_query_sharded}"

cmd=(
  "$PYTHON_BIN" "scripts/4-deferral/dspy-calibrated-gemma-query-sharded.py"
  --expert "$EXPERT"
  --language "$LANGUAGE"
  --prompt-variant "$PROMPT_VARIANT"
  --prompt-style "$PROMPT_STYLE"
  --autorun "$AUTORUN"
  --uncertainty-root "$UNCERTAINTY_ROOT"
  --output-root "$OUTPUT_ROOT"
  --student-model "$STUDENT_MODEL"
  --student-api-bases "$STUDENT_API_BASES"
  --api-key "$OPENAI_API_KEY"
  --teacher-label "$TEACHER_LABEL"
  --temperature "$TEMPERATURE"
  --top-p "$TOP_P"
  --max-tokens "$MAX_TOKENS"
  --miprov2-temp "$MIPROV2_TEMP"
  --max-article-chars "$MAX_ARTICLE_CHARS"
  --num-queries "$NUM_QUERIES"
  --num-workers-per-endpoint "$NUM_WORKERS_PER_ENDPOINT"
  --seed "$SEED"
)
if [[ -n "$LIMIT_ITEMS" ]]; then cmd+=(--limit-items "$LIMIT_ITEMS"); fi
if [[ -n "$SAMPLE_ITEMS" ]]; then cmd+=(--sample-items "$SAMPLE_ITEMS"); fi
if [[ "$FORCE" == "1" ]]; then cmd+=(--force); fi
if [[ "$SKIP_ENDPOINT_CHECK" == "1" ]]; then cmd+=(--skip-endpoint-check); fi

echo "Starting sharded DSPy query"
echo "Task: ${EXPERT}/${LANGUAGE}/${PROMPT_VARIANT}/${AUTORUN}"
echo "Prompt style: ${PROMPT_STYLE}"
echo "Endpoints: ${STUDENT_API_BASES}"
printf 'Command:'
printf ' %q' "${cmd[@]}"
printf '\n'
"${cmd[@]}"
