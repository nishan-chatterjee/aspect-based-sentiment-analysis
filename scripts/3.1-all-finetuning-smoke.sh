...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --) shift; break ;;
    -h|--help) echo "Usage: $0 --gpus 0,1 --models all|xlmr,longformer --run-prefix ID -- [train flags]"; exit 0 ;;
    *) echo "Unknown launcher flag $1" >&2; exit 2 ;;
  esac
done
[[ -n "$GPUS" && -n "$RUN_PREFIX" ]] || { echo "--gpus and --run-prefix are required" >&2; exit 2; }
DATASET=""
previous=""
for argument in "$@"; do
