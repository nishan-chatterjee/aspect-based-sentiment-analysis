...
RELEASE_SHINGLE_TOKENS="${RELEASE_SHINGLE_TOKENS:-24}"
RELEASE_SCAN_MAX_MIB="${RELEASE_SCAN_MAX_MIB:-64}"
...
for index in 0 1 2 3; do
  ...
  command=(
    ...
    --release-shingle-tokens "$RELEASE_SHINGLE_TOKENS"
    --release-scan-max-mib "$RELEASE_SCAN_MAX_MIB"
    --seed "$SEED"
    --resume
    --skip-unavailable
  )
